# ==============================================================================
# AMD 5-YEAR PROJECTION — COMPLETE ENGINE v7.0
...
# v7.0: Formatting fix — Assumptions tab row heights are now dynamic (est_row_height).
#        Previously hardcoded at 28pt (~2 lines), which clipped long narratives.
#        Dynamic heights calculated from character count vs 90-char column width.
#        Vertical alignment changed from center → top so text starts at row top.
#        All DATA BLOCK constants and narratives unchanged from v6.
...
VERSION      = "v8.0 — Q1 2026 + All Confirmed Deals (OpenAI/Meta/Oracle/HUMAIN/DoE)"
GENERATED    = "May 2026"
...
def build_projection(wb):
    ...
    foot.value = (f"Source: AMD FY2025 actual ${BASE_REV:.2f}B | "
                  ...
                  f"Non-GAAP basis | Script: build_amd_projection_engine_v7.py")
...
def build_probability(wb):
    ...
    foot.value = (f"Built: {GENERATED} | Entry ${ENTRY_PRICE} | "
                  f"Shares {SHARES}B | Non-GAAP | Script: build_amd_projection_engine_v7.py")
    foot.font = Font(name="Arial", size=7, color=MUTED, italic=True)
    foot.alignment = Alignment(horizontal="left")
...
def build_assumptions(wb):
    ...
    # Product roadmap: MI350/MI355X shipping (CDNA 4, 3nm, 288GB HBM3E; MI355X = HPC/Sovereign variant) →
    #   MI450 H2 2026 production (MI400 Series, CDNA 5, HBM4, 432GB) →
    #   MI430X = MI400 Series HPC (DoE Discovery 2028) | MI455X = likely MI400/500 training/inference 2027.
    ...
    # Confirmed deals: HUMAIN $10B/500MW (May 13, 2025; Saudi Arabia sovereign AI, full AMD stack, 5-year deployment).
    #                  OpenAI 6GW total (Oct 6, 2025; 1GW initial H2 2026 MI450 Series, 160M warrants).
    #                  Oracle 50K GPUs Q3 2026 (Oct 14, 2025; MI450 Series + EPYC Venice + Pensando Vulcano, Helios rack).
    #                  DoE/ORNL: Lux AI (MI355X + EPYC + Pensando, early 2026) + Discovery (MI430X + EPYC Venice, 2028). $1B.
    ...
    BULL_REV_NOTES = [
        ...
        "H2 acceleration driven by: OpenAI 6GW total (1GW initial H2 2026 MI450 Series, 160M warrants, Oct 6 2025). "
        "Meta 6GW total (1GW initial H2 2026 custom MI450-based GPU + EPYC Venice, 160M warrants, Feb 24 2026). "
        "Oracle 50K GPUs Q3 2026 (MI450 Series + EPYC Venice + Pensando Vulcano, Helios rack, Oct 14 2025). "
        "DoE/ORNL Lux AI (MI355X HPC variant + EPYC + Pensando, early 2026) — first US AI Factory for science. "
        "HUMAIN $10B / 500MW sovereign AI infrastructure (May 2025, Saudi Arabia, multi-exaflop by early 2026). "
        "Total confirmed backlog 12GW+ commercial + $10B Sovereign + $1B DoE — demand risk eliminated. "
        "EPYC Turin record 46.2% server CPU revenue share; AMD Data Center $5.8B now exceeds Intel $5.1B.",
        "MI450 full production volume year (MI400 Series, CDNA 5, HBM4, 432GB). "
        "OpenAI/Meta 6GW ramps scale toward full commitment. HUMAIN 500MW deployment Year 2 (~$2B+ revenue contribution). "
        "MI455X 2027 training/inference product enters production (MI400 Series or next-gen). "
        "EPYC Verano enters 1:1 CPU:GPU market. AMD Data Center at ~$8B+/quarter run rate.",
        "Inference explosion — MI400 Series HBM4 bandwidth advantage vs Nvidia. "
        ...
        "Sovereign AI deployed globally. DoE/ORNL Discovery supercomputer (MI430X + EPYC Venice) arrives 2028 — "
        "validates MI400 Series in the most demanding HPC workloads. ROCm achieving enterprise software parity. "
        "Server CPU TAM $90B+ at AMD 35%+ share. HUMAIN 500MW Year 4 sustaining. "
        "OpenAI/Meta expanding beyond initial 1GW tranches toward full 6GW.",
        ...
        "DoE/ORNL Discovery (MI430X, EPYC Venice) enters operations 2029 — Sovereign AI compounding. "
    ...
    BEAR_REV_NOTES = [
        ...
        "The signed 1GW initial orders deploy but ramp toward full 6GW is deferred to next-generation MI400 Series. "
        "HUMAIN 500MW deployment proceeds on schedule (sovereign market less ROCm-sensitive) but commercial "
        "hyperscaler revenue stalls despite confirmed backlog. AMD Data Center GPU growth disappointment.",
    ...
    ws.merge_cells(f"A{ROW}:C{ROW}")
    foot = ws[f"A{ROW}"]
    foot.value = (f"Script: build_amd_projection_engine_v7.py | Built: {GENERATED} | "
...
def build_workflow(wb):
    ...
    wf_rows = [
        ...
        ("Step 3 — Update Projection",
         ...
         "→ Pull latest build_amd_projection_engine_v*.py from GitHub → update DATA BLOCK → rebuild → present for download"),
        ("Full Earnings Day",
    ...
    link_rows = [
        ...
        ("build_amd_projection_engine_v7.py",
         "GitHub: tonyvasquez1/bmnr-dashboard — branch claude/setup-amd-financials-q7F3t (all 5 deals)"),
        ("build_amd_income_v3.py",
         "GitHub: tonyvasquez1/bmnr-dashboard — Q1 2026 income statement (all 5 deals update)"),
        ("AMD_MANIFEST.txt",
    ...
    version_rows = [
        ...
        ("v10 (script v5) — May 2026",
         ...
         "buyback). Probabilities: Bull 35%→45%, Base 45%→40%, Bear 20%→15%. Bear reframed to execution risk. "
         "Script: build_amd_projection_engine_v5.py."),
        ("v11 (script v6) — May 2026",
         "Full confirmed deal stack: HUMAIN $10B/500MW Sovereign AI (May 13 2025, Saudi Arabia, full AMD stack, "
         "5-year deployment, multi-exaflop by early 2026). AMD + DoE/ORNL (Oct 27 2025): Lux AI "
         "(MI355X + EPYC + Pensando, early 2026) + Discovery (MI430X + EPYC Venice, 2028). $1B combined. "
         "Product corrections: MI430X confirmed as MI400 Series HPC (not MI500). MI355X introduced as "
         "HPC variant of MI350/MI400 family. BULL narratives updated for Sovereign AI 2026-2030 revenue "
         "streams. All DATA BLOCK constants unchanged (SHARES=1.750B, Bull 45%/Base 40%/Bear 15%). "
         "Script: build_amd_projection_engine_v7.py."),
        ("v12 (script v7) — May 2026",
         "Assumptions tab formatting fix: row heights changed from hardcoded 28pt (~2 visible lines) "
         "to dynamic est_row_height() based on character count vs 90-char column width. "
         "Vertical alignment changed center → top so narratives read from row top. "
         "All DATA BLOCK constants, narratives, and tab structure unchanged. "
         "Script: build_amd_projection_engine_v7.py."),
    ]
...
def build_data_sources(wb):
    ...
    # CONFIRMED DEALS — ALL FIVE
    section_header(4, "CONFIRMED DEALS — PRIMARY SOURCES (ALL FIVE)")
    col_headers(5)
    deal_sources = [
        ("AMD + HUMAIN $10B Sovereign AI Collaboration",
         "May 13, 2025. $10B total investment over 5 years. 500MW AI compute capacity. "
         "Full AMD stack: Instinct GPUs, EPYC CPUs, Pensando DPUs, Ryzen AI, ROCm. "
         "Saudi Arabia-based, global AI infrastructure. Multi-exaflop capacity by early 2026. "
         "Largest revenue commitment by dollar value in AMD's history. "
         "Source for Sovereign AI revenue stream in Bull/Base assumptions 2026-2030.",
         "AMD Newsroom — AMD and HUMAIN Form Strategic $10B Collaboration (May 13, 2025)"),
        ("AMD + DoE/ORNL Sovereign AI (Lux + Discovery)",
         "Oct 27, 2025. Two supercomputers at Oak Ridge National Laboratory. $1B combined (public+private). "
         "Lux AI: MI355X GPUs (HPC variant of MI350/MI400 Series) + EPYC + Pensando, w/ Oracle Cloud + HPE, early 2026. "
         "Discovery: MI430X (confirmed MI400 Series HPC accelerator) + EPYC Venice, 2028 delivery/2029 ops. "
         "Source for MI355X product introduction, MI430X as MI400 Series (not MI500) correction, "
         "Sovereign AI government revenue in Bull case, DoE/ORNL validation of ROCm for HPC.",
        ...
        ("AMD + Oracle AI World Announcement",
         ...
         "MI450 Series accelerators + EPYC Venice CPUs + Pensando Vulcano DPUs. Helios rack. "
         "Expanding to 200K+ GPUs in 2027. Source for Oracle Cloud 2026 GPU revenue ramp "
         "and MI350/MI355X at Oracle scale narrative.",
        ...
        ("AMD + Meta Expanded Strategic Partnership",
         ...
         "and 12GW confirmed commercial pipeline total in probability rationale.",
    ...
    # AMD OFFICIAL
    section_header(12, "AMD OFFICIAL SOURCES")
    col_headers(13)
    amd_sources = [
        ...
        ("AMD Newsroom / CES 2026",
         "MI400/MI500 roadmap. OpenAI/Oracle/Meta/HUMAIN/DoE hyperscaler/sovereign deals. ROCm roadmap.",
    ...
    # AMD PRODUCT ROADMAP
    section_header(20, "AMD PRODUCT ROADMAP SOURCES")
    col_headers(21)
    roadmap_sources = [
        ("MI350 / MI355X Launch",
         "MI350 shipping. CDNA 4, 3nm, 288GB HBM3E. Oracle Cloud at scale. "
         "MI355X = HPC variant (Lux AI supercomputer, early 2026, per DoE/ORNL Oct 2025 press release). "
         "Source for 2026 GPU revenue driver and Sovereign AI product roadmap assumptions.",
        ...
        ("MI430X — Confirmed MI400 Series (DoE/ORNL)",
         "MI430X confirmed as MI400 Series HPC accelerator (per DoE/ORNL Oct 27 2025 press release). "
         "Powers Discovery supercomputer at ORNL, delivery 2028, operations 2029. "
         "Sovereign AI and HPC-optimized. EPYC Venice companion CPU. "
         "Source for 2028-2029 Sovereign AI / HPC revenue in Bull case.",
         "AMD Newsroom — AMD Powers U.S. Sovereign AI Factory Supercomputers (Oct 27, 2025)"),
        ("MI455X / Next-Gen Roadmap — WCCFtech",
         "MI455X (training/inference) reported as 2027 product. Exact product family designation "
         "(MI400 or MI500 Series) not yet confirmed by AMD official press release. "
         "Source for 2027 Bull case GPU product mix and NI margin expansion assumptions.",
        ...
        ("EPYC Verano / Venice CPU Roadmap — Tom's Hardware",
         "EPYC Verano roadmap (1:1 CPU:GPU market). Venice confirmed in DoE Discovery system. "
    ...
    section_header(29, "COMPETITIVE INTELLIGENCE")
    col_headers(30)
    comp_sources = [
    ...
    for i, (name, desc, link) in enumerate(comp_sources):
        src_row(31 + i, name, desc, link, bg=WHITE if i % 2 == 0 else GRAY)

    # SECTOR / MACRO
    section_header(35, "SECTOR & MACRO SOURCES")
    col_headers(36)
    macro_sources = [
    ...
    for i, (name, desc, link) in enumerate(macro_sources):
        src_row(37 + i, name, desc, link, bg=WHITE if i % 2 == 0 else GRAY)

    # CONSENSUS ESTIMATES
    section_header(40, "CONSENSUS ESTIMATES")
    col_headers(41)
    consensus_sources = [
        ("Yahoo Finance — AMD",
    ...
    for i, (name, desc, link) in enumerate(consensus_sources):
        src_row(42 + i, name, desc, link, bg=WHITE if i % 2 == 0 else GRAY)

    # HISTORICAL DATA
    section_header(47, "HISTORICAL DATA")
    col_headers(48)
    hist_sources = [
    ...
    for i, (name, desc, link) in enumerate(hist_sources):
        src_row(49 + i, name, desc, link, bg=WHITE if i % 2 == 0 else GRAY)

    # GOOGLE DRIVE
    section_header(54, "GOOGLE DRIVE")
    col_headers(55)
    drive_sources = [
        ("AMD Financials — Master Folder",
    ...
    for i, (name, desc, link) in enumerate(drive_sources):
        src_row(56 + i, name, desc, link, bg=WHITE if i % 2 == 0 else GRAY)
...
def self_test(out_path):
    ...
    if not os.path.exists(out_path):
        ...
        found_openai_assume = found_meta_assume = found_humain_assume = False
        for row in ws_a.iter_rows(values_only=True):
            for c in row:
                if isinstance(c, str):
                    ...
                    if "HUMAIN" in c:      found_humain_assume = True
        ...
        if not found_humain_assume:
            errors.append("FAIL: Assumptions tab — HUMAIN deal reference missing")
        ...
        found_humain_ds = found_doe_ds = False
        for row in ws_d.iter_rows(values_only=True):
            for c in row:
                if isinstance(c, str):
                    ...
                    if "ORNL" in c or "DoE" in c: found_doe_ds = True
        if not found_ir:
        ...
        if not found_humain_ds:
            errors.append("FAIL: Data Sources tab — HUMAIN deal source missing")
        if not found_doe_ds:
            errors.append("FAIL: Data Sources tab — DoE/ORNL deal source missing")
    ...
    if errors:
        ...
        print(f"  All 5 deals: OpenAI/Meta/Oracle/HUMAIN/DoE confirmed in Assumptions + Data Sources")
        print(f"  MI450/MI355X/MI430X(MI400 Series)/EPYC 46.2% coherence: confirmed")
