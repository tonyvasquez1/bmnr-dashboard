# ==============================================================================
# AMD 5-YEAR PROJECTION — COMPLETE ENGINE v3.0
...
# v3.0: Assumptions updated for MI350/MI450/MI500 and EPYC CPU data from
#        Q1 2026 income statement What's New tab. Data Sources expanded to
#        match all sources cited in the income statement.
# Update only the DATA BLOCK below each quarter. Structure never changes.
# HOW TO RUN: python build_amd_projection_engine_v3.py
# ==============================================================================
...
def build_projection(wb):
    ...
    foot.value = (f"Source: AMD FY2025 actual ${BASE_REV:.2f}B | "
                  ...
                  f"Non-GAAP basis | Script: build_amd_projection_engine_v3.py")
...
def build_probability(wb):
    ...
    foot.value = (f"Built: {GENERATED} | Entry ${ENTRY_PRICE} | "
                  f"Shares {SHARES}B | Non-GAAP | Script: build_amd_projection_engine_v3.py")
    foot.font = Font(name="Arial", size=7, color=MUTED, italic=True)
    foot.alignment = Alignment(horizontal="left")
...
def build_assumptions(wb):
    ...
    ws.merge_cells(f"A{ROW}:C{ROW}")
    foot = ws[f"A{ROW}"]
    foot.value = (f"Script: build_amd_projection_engine_v3.py | Built: {GENERATED} | "
...
def build_workflow(wb):
    ...
    version_rows = [
        ...
        ("v7 (script v3) — May 2026",
         ...
         "Script: build_amd_projection_engine_v3.py."),
    ]
...
def build():
    ...
    out = "/mnt/user-data/outputs/AMD_5Year_Projection_v8.xlsx"
    wb.save(out)
