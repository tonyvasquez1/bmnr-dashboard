# ==============================================================================
# AMD 5-YEAR PROJECTION — COMPLETE ENGINE v6.0
...
# v6.0: Full confirmed deal stack update —
#        HUMAIN $10B/500MW Sovereign AI (May 13, 2025; Saudi Arabia, full AMD stack,
#          multi-exaflop by early 2026, 5-year deployment).
#        AMD + DoE/ORNL: Lux AI (MI355X + EPYC + Pensando, early 2026) +
#          Discovery (MI430X + EPYC Venice, 2028). $1B combined investment.
#        OpenAI 6GW (Oct 6, 2025), Meta 6GW (Feb 24, 2026), Oracle 50K GPUs (Oct 14, 2025).
#        Product correction: MI430X is confirmed MI400 Series (not MI500 family);
#          MI355X is a new HPC variant of the MI350/MI400 family.
#        All prior DATA BLOCK constants unchanged (SHARES=1.750B, Bull 45%/Base 40%/Bear 15%).
# Update only the DATA BLOCK below each quarter. Structure never changes.
# HOW TO RUN: python build_amd_projection_engine_v6.py
# ==============================================================================
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment

# ── DATA BLOCK — update these values each quarter ──────────────────────────────
VERSION      = "v7.0 — Q1 2026 + All Confirmed Deals (OpenAI/Meta/Oracle/HUMAIN/DoE)"
GENERATED    = "May 2026"
...
def build_projection(wb):
    ...
    foot.value = (f"Source: AMD FY2025 actual ${BASE_REV:.2f}B | "
                  ...
                  f"Non-GAAP basis | Script: build_amd_projection_engine_v6.py")
...
def build_probability(wb):
    ...
    foot.value = (f"Built: {GENERATED} | Entry ${ENTRY_PRICE} | "
                  f"Shares {SHARES}B | Non-GAAP | Script: build_amd_projection_engine_v6.py")
    foot.font = Font(name="Arial", size=7, color=MUTED, italic=True)
    foot.alignment = Alignment(horizontal="left")
...
def build_assumptions(wb):
    ...
    ws.merge_cells(f"A{ROW}:C{ROW}")
    foot = ws[f"A{ROW}"]
    foot.value = (f"Script: build_amd_projection_engine_v6.py | Built: {GENERATED} | "
...
def build_workflow(wb):
    ...
    link_rows = [
        ...
        ("build_amd_projection_engine_v6.py",
    ...
    version_rows = [
        ...
        ("v11 (script v6) — May 2026",
         ...
         "Script: build_amd_projection_engine_v6.py."),
    ]
...
def build():
    ...
    out = "/mnt/user-data/outputs/AMD_5Year_Projection_v11.xlsx"
    wb.save(out)
