...
# v3: Adds HUMAIN $10B/500MW Sovereign AI deal (May 13, 2025) and
#     AMD + DoE/ORNL Sovereign AI (Oct 27, 2025): Lux AI (MI355X, early 2026) +
#     Discovery (MI430X confirmed MI400 Series, 2028) to What's New and Data Sources.
#     Product correction: MI430X is confirmed MI400 Series (not MI500 family).
#     MI355X introduced as HPC variant of MI350/MI400 family.
#     Coherence Check updated to reference build_amd_projection_engine_v6.py.
...
def build_sources_tab(wb):
    ...
    sources = [
        # ── Confirmed deals — all five (v3 adds HUMAIN + DoE/ORNL) ────────────
        ("Strategic Deal",
         ...
         "Saudi Arabia-based, multi-exaflop by early 2026. Largest revenue commitment in AMD history. "
         "Used in: What's New HUMAIN item, Sovereign AI stream in Bull/Base projections 2026-2030, "
         "ROCm validation evidence in Bear case risk mitigation.",
         "AMD Newsroom — AMD and HUMAIN Form Strategic $10B Collaboration (May 13, 2025)"),

        ("Strategic Deal",
         "AMD + DoE/ORNL Sovereign AI (Lux + Discovery)",
         "Oct 27, 2025",
         "Two ORNL supercomputers. $1B combined (public+private). "
         "Lux AI: MI355X (HPC variant MI350/MI400 Series) + EPYC + Pensando, Oracle Cloud + HPE, early 2026. "
         "Discovery: MI430X (confirmed MI400 Series) + EPYC Venice, 2028/2029. "
         "Key source: MI430X product family correction (MI400 not MI500), MI355X introduction. "
         "Used in: What's New DoE item, MI430X MI400 Series correction in v6 projection engine, "
         "Sovereign AI government revenue stream in Bull/Base cases.",
...
def build_whats_new_tab(wb):
    ...
    t = ws["A1"]
    t.value = ("AMD — WHAT'S NEW  |  Version: Q1 2026 (All 5 Deals)  |  "
               "Script: build_amd_income_v3.py  |  Researched: May 2026")
    t.font = Font(name="Arial", bold=True, size=SZ_TITLE, color=WHITE)
    t.fill = fill(NAVY)
    t.alignment = center()
    ws.row_dimensions[1].height = 24
    ...
    items = [
        # ── AMD STRATEGIC DEALS — UPDATED IN v3 (ALL FIVE) ────────────────────
        ("AMD — Strategic",
         "HUMAIN $10B / 500MW Sovereign AI Collaboration (May 13, 2025)",
         "No prior sovereign AI agreement of this scale",
         "$10B total investment over 5 years. 500MW AI compute capacity across Saudi Arabia and global nodes. "
         "Full AMD stack: Instinct GPUs, EPYC CPUs, Pensando DPUs, Ryzen AI, ROCm open software. "
         "Multi-exaflop capacity by early 2026. Initial deployments already underway as of May 2025. "
         "Largest revenue commitment by dollar value in AMD's history.",
         "No direct Q1 2026 revenue impact (deployment underway since 2025, multi-year). "
         "Validates ROCm open software ecosystem at sovereign AI scale — critical for Bear case ROCm risk. "
         "HUMAIN full-stack deployment (GPU + CPU + DPU + software) is AMD's highest-margin revenue mix.",
         "HUMAIN $10B/5-year = ~$2B/year AMD compute revenue potential. Sovereign AI stream supports "
         "Bull/Base 2026-2030 revenue growth assumptions as a parallel engine to commercial hyperscaler deals. "
         "ROCm success at HUMAIN scale de-risks the Bear case ROCm parity concern."),

        ("AMD — Strategic",
         "AMD + DoE/ORNL Sovereign AI: Lux AI + Discovery (Oct 27, 2025)",
         "ORNL hosted Frontier (AMD-powered, world's first exascale system) — prior relationship",
         "Two next-gen supercomputers at Oak Ridge National Laboratory. $1B combined investment. "
         "Lux AI: MI355X GPUs (HPC variant of MI350/MI400 Series) + EPYC + Pensando, "
         "co-developed with Oracle Cloud + HPE, deploying early 2026. Nation's first dedicated AI Factory for science. "
         "Discovery: MI430X (confirmed MI400 Series HPC accelerator) + EPYC Venice, 2028 delivery, 2029 operations. "
         "Key product correction: MI430X is confirmed MI400 Series (per this press release) — not MI500.",
         "No Q1 2026 revenue (Lux in production ramp, Discovery 2028). "
         "MI355X introduction confirms AMD has multiple variants of the MI350/MI400 product family for different "
         "workloads (HPC vs commercial AI). DoE/ORNL validation = strongest possible proof point for ROCm HPC parity.",
         "DoE/ORNL validates MI400 Series in the most demanding HPC + Sovereign AI workloads. "
         "MI430X as MI400 Series product (arriving 2028) supports Bull case 2028-2029 revenue. "
         "Projection engine v6 corrects MI430X from MI500 to MI400 Series category. "
         "Sovereign AI government revenue stream adds $1-2B+ per year to Bull case 2027+."),
    ...
    sec.value = ("▶  COHERENCE CHECK — PROJECTION ENGINE vs INCOME STATEMENT  |  "
                 "RED = mismatch requiring update to build_amd_projection_engine_v6.py")
    ...
    ws.merge_cells(f"A{next_row}:F{next_row}")
    sub = ws[f"A{next_row}"]
    sub.value = ("Projection engine file: build_amd_projection_engine_v6.py  |  "
                 ...
                 "v6 adds: HUMAIN $10B/500MW, DoE/ORNL (Lux MI355X + Discovery MI430X), "
                 "MI430X confirmed MI400 Series (not MI500), MI355X introduced")
...
def build():
    ...
    foot.value = (
        ...
        "Script: build_amd_income_v3.py  |  Not investment advice."
    )
...
def self_test(out_path):
    ...
    if not os.path.exists(out_path):
        ...
        # Check What's New has all confirmed deals
        ws_wn = wb["What's New"]
        found_openai = found_meta = found_oracle = found_v6 = False
        found_humain_wn = found_doe_wn = False
        for row in ws_wn.iter_rows(values_only=True):
            for cell in row:
                if cell:
                    ...
                    if "projection_engine_v6" in cv:        found_v6 = True
                    if "HUMAIN" in cv:                       found_humain_wn = True
                    if "ORNL" in cv or "Discovery" in cv:   found_doe_wn = True
        ...
        if not found_v6:        errors.append("FAIL: What's New — v6 engine reference missing")
        if not found_humain_wn: errors.append("FAIL: What's New — HUMAIN deal missing")
        if not found_doe_wn:    errors.append("FAIL: What's New — DoE/ORNL deal missing")

        # Check Data Sources has all deal entries
        ws_ds = wb["Data Sources"]
        found_openai_ds = found_oracle_ds = found_humain_ds = False
        for row in ws_ds.iter_rows(values_only=True):
            for cell in row:
                if cell:
                    ...
                    if "HUMAIN" in cv:   found_humain_ds = True
        ...
        if not found_humain_ds: errors.append("FAIL: Data Sources — HUMAIN deal entry missing")
    ...
    print(f"  All 5 deals (OpenAI/Meta/Oracle/HUMAIN/DoE): confirmed in What's New")
    print(f"  v6 engine reference: confirmed in Coherence Check")
    print(f"  OpenAI/Oracle/HUMAIN: confirmed in Data Sources")
    print("=" * 60)
    return True
